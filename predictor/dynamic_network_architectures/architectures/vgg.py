...
from predictor.dynamic_network_architectures.building_blocks.plain_conv_encoder import (
    PlainConvEncoder,
)
...
class VGG(nn.Module):
    def __init__(
        self, n_classes: int, n_input_channel: int = 3, config="16", input_dimension=2
    ):
        """
        ...
        self.gap = get_matching_pool_op(
            conv_op=ops["conv_op"], adaptive=True, pool_type="avg"
        )(1)
...
class VGG16(VGG):
    def __init__(
        self, n_classes: int, n_input_channel: int = 3, input_dimension: int = 2
    ):
        super().__init__(
            n_classes, n_input_channel, config="16", input_dimension=input_dimension
        )
...
class VGG19(VGG):
    def __init__(
        ...
        super().__init__(
            n_classes, n_input_channel, config="19", input_dimension=input_dimension
        )
