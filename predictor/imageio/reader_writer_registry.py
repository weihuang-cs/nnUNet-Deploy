...
import predictor
from predictor.imageio.natural_image_reager_writer import NaturalImage2DIO
from predictor.imageio.nibabel_reader_writer import NibabelIO, NibabelIOWithReorient
from predictor.imageio.simpleitk_reader_writer import SimpleITKIO
from predictor.imageio.tif_reader_writer import Tiff3DIO
...
def recursive_find_reader_writer_by_name(rw_class_name: str) -> Type[BaseReaderWriter]:
    ret = recursive_find_python_class(join(predictor.__path__[0], "imageio"), rw_class_name, 'predictor.imageio')
    if ret is None:
        raise RuntimeError("Unable to find reader writer class '%s'. Please make sure this class is located in the "
                           "predictor.imageio module." % rw_class_name)
    else:
        return ret
